...
// src/match-extension.ts
var matchExtension = (extensions, filename) => {
  return extensions.some((ext) => filename.endsWith(ext));
};
var match_extension_default = matchExtension;

// src/group-package-changed.ts
var groupPackagesChanged = (files, ignore = [], group_regex = /^packages\/([^/]+)\/(.+)$/) => {
  return files.filter(
    (filename) => !match_extension_default(ignore, filename) && filename.match(group_regex) !== null
  ).map((filename) => {
...
var group_package_changed_default = groupPackagesChanged;
...
async function run() {
  try {
    ...
    const packages = group_package_changed_default(
